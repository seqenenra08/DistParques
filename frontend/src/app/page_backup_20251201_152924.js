...
export default function Home() {
  ...
  const getPlayerInfo = (color) => {
    ...
    if (gameState.players && gameState.players.length > 0) {
      ...
      if (gamePlayer) {
        ...
        return { name: gamePlayer.name || `Jugador ${color}`, color: color };
      }
    }
    ...
    if (activePlayer) {
      ...
      return { name: activePlayer.name || `Jugador ${color}`, color: color };
    }
    
    // Fallback a nombres descriptivos por color
    const colorNames = {
      red: 'Jugador Rojo',
      blue: 'Jugador Azul', 
      green: 'Jugador Verde',
      yellow: 'Jugador Amarillo'
    };
    
    const fallbackName = colorNames[color] || `Jugador ${color}`;
  ...
  const getActivePlayerColors = () => {
    // Si estamos en juego, obtener TODOS los jugadores del gameState (incluir bots para mostrar sus fichas)
    if (gameState?.players && Array.isArray(gameState.players)) {
      console.log('========================================');
      console.log('[getActivePlayerColors] 🎨 ANÁLISIS DE JUGADORES');
      console.log('[getActivePlayerColors] Total jugadores en gameState:', gameState.players.length);
      console.log('[getActivePlayerColors] Jugadores completos:', JSON.stringify(gameState.players, null, 2));
      
      const colors = gameState.players
        .filter(p => {
          const hasColor = !!p.color;
          console.log(`[getActivePlayerColors] ✓ Jugador: ${p.name}, Color: ${p.color}, Tiene color: ${hasColor}, ID: ${p.player_id}`);
          return hasColor;
        })
        .map(p => {
          console.log(`[getActivePlayerColors] ✅ INCLUIDO - Player: ${p.name}, Color: ${p.color}`);
          return p.color;
        });
      console.log('[getActivePlayerColors] 🎯 Colores finales:', colors);
      console.log('[getActivePlayerColors] 🎯 Total colores:', colors.length);
      console.log('========================================');
      return colors;
    }
    ...
    // Si tenemos activePlayers, usarlos (incluir todos para que se muestren)
    if (activePlayers.length > 0) {
      console.log('[getActivePlayerColors] Usando activePlayers:', activePlayers.length);
      const colors = activePlayers
        .filter(p => p.color) // Solo verificar que tengan color
        .map(p => p.color);
      console.log('[getActivePlayerColors] Colores de activePlayers:', colors);
      return colors;
    }
  ...
  useEffect(() => {
    ...
    socket.on('dice_rolled', (data) => {
      ...
      console.log('[dice_rolled EVENT] CurrentPlayer:', data.estado?.jugador_actual);
      console.log('[dice_rolled EVENT] MyPlayerColor:', myPlayerColor);
      console.log('[dice_rolled EVENT] start_phase:', data.start_phase);
      console.log('[dice_rolled EVENT] is_doubles:', data.is_doubles);
      console.log('[dice_rolled EVENT] can_retry:', data.can_retry);
      console.log('[dice_rolled EVENT] turn_passed:', data.turn_passed);
      console.log('[dice_rolled EVENT] attempts:', `${data.attempts_used}/${data.attempts_used + data.attempts_remaining}`);
      
      // ✅ Si no es mi turno (es el bot u otro jugador), activar animación visual
      const currentPlayer = data.estado?.jugador_actual;
      const isMyTurn = currentPlayer === myPlayerColor;
      if (!isMyTurn) {
      ...
      setTimeout(() => {
        // Configurar valores de dados
        console.log('[dice_rolled] 🎲🎲 Configurando diceValue a:', data.dados);
        setDiceValue(data.dados);
        setIsRolling(false);
        
        // Actualizar estado del juego
        if (data.estado) {
          console.log('[DEBUG] Actualizando gameState con:', data.estado);
          
          const newGameState = {
            ...gameState,
            currentPlayer: data.estado.jugador_actual,
            jugadores: data.estado.jugadores,
            // Información de fase de inicio
            startPhase: data.start_phase || false,
            currentAttempts: data.attempts_used || 0,
        ...
        // ==========================================
        // MANEJAR FASE DE INICIO (3 intentos)
        // ==========================================
        if (data.start_phase === true) {
          console.log('[dice_rolled] 📍 FASE DE INICIO - todas las fichas en cárcel');
          ...
          // CASO 3: No sacó par pero puede intentar de nuevo
          if (data.can_retry === true) {
            console.log('[dice_rolled] ⚠️ No sacó par, puede intentar de nuevo');
            setCanMove(false);
            const attemptsRemaining = data.attempts_remaining || 0;
            setMessage(`⚠️ No sacaste par. Te quedan ${attemptsRemaining} ${attemptsRemaining === 1 ? 'intento' : 'intentos'}.`);
        ...
        // ==========================================
        // JUEGO NORMAL (fuera de fase de inicio)
        // ==========================================
        console.log('[dice_rolled] 🎮 Modo de juego normal');
        
        if (data.no_moves) {
          console.log('[dice_rolled] ⚠️ No hay movimientos posibles');
          setCanMove(false);
          setMessage('⚠️ No hay movimientos posibles');
          setTimeout(() => {
            setDiceValue(null);
            setMessage('');
          }, 2000);
        } else {
          console.log('[dice_rolled] ✅ Puede mover');
          setCanMove(true);
        }
    ...
    socket.on('game_created', (data) => {
      ...
      if (data.success) {
        ...
        if (data.game_state && data.game_state.players) {
          console.log('========================================');
          console.log('[game_created] 🎮 PROCESANDO JUGADORES DEL SERVIDOR');
          console.log('[game_created] Total players recibidos:', data.game_state.players.length);
          const newActivePlayers = data.game_state.players.map((p, idx) => {
            const isBot = (p.player_id || p.id).startsWith('bot_');
            console.log(`[game_created] ${idx+1}. Nombre: ${p.name}, Color: ${p.color}, ID: ${p.player_id}, IsBot: ${isBot}`);
            return {
              id: p.player_id || p.id,
              name: p.name,
              color: p.color,
          ...
          console.log('[game_created] 📊 Resumen de activePlayers:');
          newActivePlayers.forEach((p, i) => {
            console.log(`  ${i+1}. ${p.name} - ${p.color} (ID: ${p.id})`);
          });
          console.log('[game_created] Total activePlayers creados:', newActivePlayers.length);
          console.log('========================================');
    ...
    socket.on('game_state_updated', (data) => {
      ...
      console.log('[game_state_updated EVENT] Cambio de turno:', data.cambio_turno);
      console.log('[game_state_updated EVENT] ========================================');
      
      if (data.game_state) {
        const previousPlayer = gameState.currentPlayer;
        const newPlayer = data.game_state.current_player;
        
        // Actualizar el estado del juego con la información más reciente
        const newGameState = {
          ...data.game_state,
          currentPlayer: newPlayer,
          canMove: false, // Siempre resetear a false cuando se actualiza el estado
        ...
        console.log('[game_state_updated] Turno anterior:', previousPlayer, '-> Nuevo:', newPlayer);
        setGameState(newGameState);
        
        // Si el turno cambió, limpiar estados locales
        if (data.cambio_turno || (previousPlayer && previousPlayer !== newPlayer)) {
          console.log('[game_state_updated] ✅ Turno cambió - limpiando estados locales');
          setDiceValue(null);
          setCanMove(false);
          setIsRolling(false);
          setMaxPiecesToRelease(0);
          setMessage('');
          
          // Mostrar transición de turno si es mi turno
          if (newPlayer === myPlayerColor) {
            console.log('[game_state_updated] 🎯 Es mi turno ahora!');
            setTimeout(() => {
              showTurnTransition(newPlayer, 'Tu turno');
            }, 300);
          }
        }
      }
  ...
  const getCurrentGameState = () => {
    ...
    console.log('========================================');
    console.log('[getCurrentGameState] 📊 ESTADO DEL JUEGO');
    console.log('[getCurrentGameState] Total jugadores:', baseState.players?.length);
    console.log('[getCurrentGameState] Jugador actual:', baseState.currentPlayer);
    console.log('[getCurrentGameState] Lista de jugadores:');
    baseState.players?.forEach((p, i) => {
      console.log(`  ${i+1}. Nombre: ${p.name}, Color: ${p.color}, ID: ${p.player_id}, Fichas: ${p.pieces?.length}`);
    });
    console.log('========================================');
  ...
  const handlePieceClick = (pieceId) => {
    ...
    } else if (diceValue && Array.isArray(diceValue) && diceValue.length === 2) {
      // Dados lanzados pero sin availableMoves - mostrar selector con opciones individuales y suma
      const moves = [diceValue[0], diceValue[1]];
      if (diceValue[0] !== diceValue[1]) {
        moves.push(diceValue[0] + diceValue[1]); // Agregar suma si no son dobles
      }
  ...
  const handleStartGame = (gameConfig) => {
    ...
    if (connected) {
      ...
      console.log('========================================');
      console.log('[DEBUG] 🎮 ESTADO INICIAL DEL JUEGO OFFLINE');
      console.log('[DEBUG] Total jugadores:', initialState.players.length);
      console.log('[DEBUG] Jugadores:');
      initialState.players.forEach((p, i) => {
        console.log(`  ${i+1}. ${p.name} - Color: ${p.color} - ID: ${p.player_id}`);
      });
      console.log('========================================');
    ...
    <div className={styles.container}>
      ...
      <main className={styles.main}>
        ...
        <div className={styles.status}>
          ...
          <p>Jugadores activos: {getActivePlayerColors().length} / {gameState?.players?.length || 0} en gameState</p>
          <div className={styles.playersInfo}>
            {(() => {
              const colors = getActivePlayerColors();
              console.log('[RENDER STATUS] 🎨 Renderizando badges, colores:', colors);
              return colors.map(color => {
                const playerInfo = getPlayerInfo(color);
                console.log('[RENDER STATUS] Badge para color:', color, 'nombre:', playerInfo.name);
                return (
          ...
          <div className={styles.sidePanel}>
            ...
            <div className={styles.currentTurn} style={{
              ...
              {gameState.currentPlayer && (
                <div style={{
                  ...
                  fontSize: '0.9rem',
                  fontWeight: '600',
                  backgroundColor: gameState.currentPlayer === 'red' ? 'rgba(220, 38, 38, 0.2)' :
                                  gameState.currentPlayer === 'blue' ? 'rgba(37, 99, 235, 0.2)' :
                                  gameState.currentPlayer === 'green' ? 'rgba(22, 163, 74, 0.2)' : 'rgba(202, 138, 4, 0.2)',
                  color: gameState.currentPlayer === 'red' ? '#fca5a5' :
                        gameState.currentPlayer === 'blue' ? '#93c5fd' :
                        gameState.currentPlayer === 'green' ? '#86efac' : '#fde047',
                  border: `2px solid ${gameState.currentPlayer === 'red' ? '#dc2626' :
                                      gameState.currentPlayer === 'blue' ? '#2563eb' :
                                      gameState.currentPlayer === 'green' ? '#16a34a' : '#ca8a04'}`,
                  marginBottom: '8px'
                }}>
                  ...
                  <span style={{ textTransform: 'capitalize' }}>
                    {gameState.currentPlayer === 'red' ? 'Rojo' :
                     gameState.currentPlayer === 'blue' ? 'Azul' :
                     gameState.currentPlayer === 'green' ? 'Verde' : 'Amarillo'}
                  </span>
                </div>
              )}
            ...
            <div className={styles.playersList}>
              ...
              <div className={styles.playersContainer}>
                {(() => {
                  const colors = getActivePlayerColors();
                  console.log('[RENDER] 🎨 Colores para renderizar en panel:', colors);
                  console.log('[RENDER] 🎨 Total a renderizar:', colors.length);
                  return colors;
                })().map((color, index) => {
                  const playerInfo = getPlayerInfo(color);
